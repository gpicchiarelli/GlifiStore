...
`package-upgrade` reports `NOT_APPLICABLE_INITIAL_BASELINE` while no previous release
exists to upgrade from. It is not a pass, and it does not promote the backend to
`UPGRADE_VERIFIED`. Upstream Debian acceptance is not claimed and not in scope here.
