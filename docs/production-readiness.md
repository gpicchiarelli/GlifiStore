...
- [ ] **GATE-PACKAGE-ADMISSION** — Package upgrade baselines and package artifacts are admitted by exact sealed bytes
  ...
  Residual risk: Admission is wired fail-closed for tool execution and retains blockers; Linux nightly 34666166603 retained package-installed cross-SDK PASS for deb and rpm, but admitted stays false until package-upgrade is exercised against a sealed SemVer N-1 predecessor on required backends; FreeBSD/OpenBSD still adapt release_evidence without package-matrix upgrade rows; no annotated release has retained a positive admission yet
