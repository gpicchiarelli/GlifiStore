...
- [ ] **GATE-PACKAGE-LIFECYCLE** — Package backends declare and prove their lifecycle through honest evidence
  ...
  Residual risk: Retained nightly Package CI run 34662210614 (tip 58167e8) proved deb and rpm LIFECYCLE_VERIFIED with service-lifecycle PASS under systemd PID 1; overall result remains NOT_RUN without a sealed candidate, and required_for_release stays false. No retained MacPorts/Homebrew native lifecycle; Homebrew brew services is exercised when native runs, otherwise OPEN_GATE with MacPorts; every upstream acceptance stays OPEN_GATE; BSD external consumer is implemented but unretained on a tagged release; deb, rpm, macports and homebrew remain required_for_release false
