...
- [ ] **GATE-PACKAGE-LIFECYCLE** — Package backends declare and prove their lifecycle through honest evidence
  ...
  Residual risk: Linux container dispatch now starts systemd as PID 1 when cgroup/privileged allow it (fallback keeps service-lifecycle BLOCKED); awaiting a retained nightly PASS for that row before claiming LIFECYCLE_VERIFIED. No retained MacPorts/Homebrew native lifecycle; MacPorts/Homebrew service rows and every upstream acceptance stay OPEN_GATE; BSD external consumer is implemented but unretained on a tagged release; deb, rpm, macports and homebrew remain required_for_release false
