...
- [ ] **GATE-PACKAGE-ADMISSION** — Package upgrade baselines and package artifacts are admitted by exact sealed bytes
  ...
  Residual risk: Admission is wired fail-closed for tool execution and retains blockers, but admitted stays false until a package-installed cross-SDK matrix PASSes and package-upgrade is exercised against a sealed SemVer N-1 predecessor; Linux nightly/release container rows now package language SDK archives on the host and install matrix toolchains in-image so the retained installed-sdk-matrix.json can move past NOT_RUN; FreeBSD/OpenBSD still adapt release_evidence without package-matrix upgrade rows; no annotated release has retained a positive admission yet
