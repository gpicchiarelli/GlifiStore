...
- [ ] **GATE-PACKAGE-ADMISSION** — Package upgrade baselines and package artifacts are admitted by exact sealed bytes
  ...
  Residual risk: Admission is wired fail-closed for tool execution and retains blockers, but admitted stays false until a package-installed cross-SDK matrix PASSes and package-upgrade is exercised against a sealed SemVer N-1 predecessor; Linux lifecycle now emits installed-sdk-matrix.json after install (honest NOT_RUN when sealed SDK archives are absent) and admission prefers that retained report over inventing a bare NOT_RUN; FreeBSD/OpenBSD still adapt release_evidence without package-matrix upgrade rows; no annotated release has retained a positive admission yet
...
- [ ] **GATE-PACKAGE-LIFECYCLE** — Package backends declare and prove their lifecycle through honest evidence
  ...
  Residual risk: Linux container dispatch tries systemd-as-PID-1 create variants (cgroupns=host, then private+docker.slice) before falling back so service-lifecycle stays BLOCKED only when the host cannot; awaiting a retained nightly PASS before claiming LIFECYCLE_VERIFIED. No retained MacPorts/Homebrew native lifecycle; Homebrew brew services is exercised when native runs, otherwise OPEN_GATE with MacPorts; every upstream acceptance stays OPEN_GATE; BSD external consumer is implemented but unretained on a tagged release; deb, rpm, macports and homebrew remain required_for_release false
