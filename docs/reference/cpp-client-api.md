...
`execute_worker_pipelines()` takes a vector of length `worker_count()`: index `i` is the pipeline for
Worker `i` (empty slots are skipped). Every key in slot `i` must route to Worker `i`. The return value
is one response vector per Worker. Same non-atomic fan-out and shared deadline as `execute_batch`.

Optional `RequestOptions{.timeout = …}` overrides `ClientConfig::request_timeout_ms` for a single
`get` / `put` / `erase` / `ping` / `backup` / `execute_pipeline` / `execute_batch` /
`execute_worker_pipelines`. Automatic retries reuse the
