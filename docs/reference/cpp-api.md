...
```cpp
struct Store::EraseItem {
    std::string_view key;
};

auto erase_batch(std::span<const EraseItem> items) -> std::vector<Status>;
```

Same positional, per-owner, non-transactional rules as `put_batch` for ERASEs. Key views are
borrowed only for the call. Successful erases are visible when the method returns.
