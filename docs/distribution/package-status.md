...
- .github/workflows/package-ci.yml retains this backend's evidence per profile and GATE-PACKAGE-LIFECYCLE cites that workflow, but the gate is open (IMPLEMENTATA), no retained run has executed the container lifecycle, and required_for_release stays false.
