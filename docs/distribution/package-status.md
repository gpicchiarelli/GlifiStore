...
| `rpm-fedora-41-amd64` | `rpm` | `fedora-41` | `amd64` | `ubuntu-24.04` | `quay.io/fedora/fedora:41` | `sha256:893f7eeffce8…` | `main`, `nightly`, `release` |
...
| `package-upgrade` | `upgrade` | `package` | upgrade from the sealed N-1 package and prove dataset continuity |
...
- .github/workflows/package-ci.yml retains this backend's evidence per profile. Retained nightly container runs reach FUNCTIONALLY_VERIFIED; service-lifecycle needs systemd as PID 1 (container dispatch now starts linux-systemd-pid1.sh with cgroup + privileged, falling back to BLOCKED when the host cannot). required_for_release stays false until a gate promotes this backend.
