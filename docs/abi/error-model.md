...
asserts known-not-committed (today: facade validation rejects before engine entry). I/O, internal,
unavailable, integrity, `GS_NOT_FOUND` from the engine (including same-key FIFO ERASE), and other
ambiguous failures are reported as `GS_MUTATION_INDETERMINATE` unless the facade has stronger
evidence. This may be more conservative than the internal failure but never authorizes an unsafe
replay.
