...
`put_batch`, `erase`, and `erase_batch`, owning result values, configuration value types, typed error categories,
...
`erase_batch` follows the same positional, per-owner, non-transactional rules for ERASEs.
