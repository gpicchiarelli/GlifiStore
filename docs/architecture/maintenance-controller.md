...
5. **Emergency mutation gate**: publish `mutations_rejected`; `Store::put` / `Store::erase` (and
   ...
   `Store::put_batch` / `Store::erase_batch`) return `ErrorCode::storage_exhausted` with a stable
   message. `get`, `flush`, `compact`, and `close` continue. No mutation queue. The sync Writer
   fast path skips only the nested `OperationGuard` RMW — not this gate — so mid-batch / TOCTOU
   arming still rejects before Store append.
