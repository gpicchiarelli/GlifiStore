#ifndef GLIFISTORE_ABI_GLIFISTORE_H
#define GLIFISTORE_ABI_GLIFISTORE_H

#include <stddef.h>
#include <stdint.h>

#if defined(_WIN32)
#  if defined(GLIFISTORE_ABI_BUILD)
...
#define GS_ABI_VERSION_MINOR 1u
...
typedef struct gs_erase_request {
    uint32_t struct_size;
    uint32_t reserved;
    gs_bytes_view key;
} gs_erase_request;
...
GS_API gs_status gs_store_erase_batch(gs_store *store, const gs_erase_request *requests,
                                      size_t request_count, gs_mutation_result *results) GS_NOEXCEPT;
...
#endif /* GLIFISTORE_ABI_GLIFISTORE_H */
