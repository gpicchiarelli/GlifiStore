...
GLYPHA_TEST("daemon dump-config prints resolved effective settings") {
    ...
    GLYPHA_REQUIRE(dump.find("close-drain-deadline-ms=\n") != std::string::npos);
...
GLYPHA_TEST("daemon close-drain-deadline-ms sets StoreConfig optional bound") {
    const std::array arguments{"glyphastored", "--close-drain-deadline-ms", "1500"};
    const auto parsed = parse(arguments);
    GLYPHA_REQUIRE(parsed.has_value());
    GLYPHA_REQUIRE(parsed->store.close_drain_deadline_ms.has_value());
    GLYPHA_REQUIRE(*parsed->store.close_drain_deadline_ms == 1500U);
    const auto dump = glyphastore::server::format_daemon_config_dump(*parsed);
    GLYPHA_REQUIRE(dump.find("close-drain-deadline-ms=1500\n") != std::string::npos);
}
