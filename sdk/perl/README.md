...
`$client->health`, `$client->ready`, and `$client->stats` issue wire `HEALTH` / `READY` / `STATS`
with empty key and value. AF_UNIX dial is supported; combining AF_UNIX with TLS is refused
fail-closed.
