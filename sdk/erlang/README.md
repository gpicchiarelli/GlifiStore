# GlifiStore Erlang client

Native Erlang/OTP client for GlifiStore wire protocol v2.

**Concurrency model:** `glifistore_client` is a coordinator only (config, routing
metadata, request-ID allocation, health/epoch, lifecycle). Normal request I/O is
delegated to monitored processes that call the per-Worker `glifistore_conn`
...
Version: `glifistore_version:version/0` (must match repository `VERSION`)  
...
#{outcome := committed} = glifistore_client:put(Client, <<"session\00042">>, <<"payload">>),
{ok, Value} = glifistore_client:get(Client, <<"session\00042">>),
...
| Module | Role |
| --- | --- |
| `glifistore_protocol` | Wire codec, FNV-1a routing, golden fixture tests |
| `glifistore_error` | Structured client-semantics v1 errors |
| `glifistore_conn` | Per-Worker TCP/TLS connection `gen_server` (serial I/O) |
| `glifistore_conn_sup` | OTP supervisor for Worker conns (`temporary`, intensity 0) |
| `glifistore_client` | Public sync API + coordinator (noreply I/O delegation) |
| `scripts/glifistore-interop` | CLI for `scripts/test-sdk-interop.sh` |
...
| Serialize per Worker | `glifistore_conn` queues exchanges; pipelines use atomic `run_pipeline/4` (send+collect in one call) |
...
| Conn supervision | `glifistore_conn_sup`: one temporary child per Worker, intensity 0 (no auto-restart) |
...
Public lifecycle probes match the other official SDKs: `health/1,2`, `ready/1,2`, and
`stats/1,2` return ASCII `GlifiStore/live`, `GlifiStore/ready`, and a `GlifiStore/stats…`
body on OK. `unix_socket_path` dials AF_UNIX and is mutually exclusive with TLS (ADR 0029).
...
Against a local `glifistored` (same matrix as Python/Perl/Go/Ruby):
...
`glifistore_client:backup(Client, Destination[, Opts])` issues wire `BACKUP`; `Destination` is a
...
{ok, Client} = glifistore_client:connect(Config).
```
...
```bash
sdk/erlang/scripts/glifistore-interop.escript --port "$PORT" get --key-hex 6161
```
