...
erase_absent_is_rejected(_Config) ->
    ...
    try
        {ok, Client} = glyphastore_client:connect(#{port => glyphastore_fake_server:port(Server)}),
        #{outcome := rejected, error := Err} = glyphastore_client:erase(Client, <<"missing">>),
        not_found = maps:get(category, Err),
...
batch_erase_absent_is_failed_rejected(_Config) ->
    ...
    try
        {ok, Client} = glyphastore_client:connect(#{port => glyphastore_fake_server:port(Server)}),
        {ok, Responses} =
            glyphastore_client:execute_batch(Client, [
                #{opcode => erase, key => <<"missing">>}
            ]),
        failed = maps:get(outcome, lists:nth(1, Responses)),
        Err = maps:get(error, lists:nth(1, Responses)),
...
worker_pipelines_erase_absent_is_failed_rejected(_Config) ->
    ...
    try
        {ok, Client} = glyphastore_client:connect(#{port => glyphastore_fake_server:port(Server)}),
        {ok, Results} =
            glyphastore_client:execute_worker_pipelines(Client, [
                [#{opcode => erase, key => <<"missing">>}]
            ]),
...
health_ready_stats_and_routing(_Config) ->
    ...
    try
        {ok, Client} = glyphastore_client:connect(#{port => glyphastore_fake_server:port(Server)}),
        true = glyphastore_client:healthy(Client),
        _Routing = glyphastore_client:routing(Client),
        {ok, <<"GlyphaStore/live">>} = glyphastore_client:health(Client),
        {ok, <<"GlyphaStore/ready">>} = glyphastore_client:ready(Client),
        {ok, Stats} = glyphastore_client:stats(Client),
        <<"GlyphaStore/stats", _/binary>> = Stats,
        glyphastore_client:close(Client)
    after
        glyphastore_fake_server:stop(Server)
    end.
...
unix_socket_refuses_tls(_Config) ->
    {error, Err} = glyphastore_client:connect(#{
        unix_socket_path => "/tmp/glyphastore-erl-uds-tls-refuse.sock",
        tls => #{enable => true}
    }),
